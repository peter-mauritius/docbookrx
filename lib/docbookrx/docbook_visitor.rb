module Docbookrx
  ...
  class DocbookVisitor
    ...
    LIST_NAMES = ['itemizedlist', 'orderedlist', 'variablelist', 'procedure', 'simplelist', 'substeps', 'stepalternatives']
    ...
    def before_traverse node, method
      ...
      when "visit_itemizedlist", "visit_orderedlist",
        "visit_procedure", "visit_substeps", "visit_stepalternatives", "visit_simplelist"
        @list_depth += 1
    ...
    def after_traverse node, method
      ...
      if at_root
        ...
        case method_name
        when "visit_itemizedlist", "visit_orderedlist", "visit_simplelist"
          @list_depth -= 1
        when "visit_table", "visit_informaltable"
          @in_table = false
        when "visit_group"
        ...
        when "visit_itemizedlist", "visit_orderedlist", "visit_table",
          "visit_informaltable", "visit_quandaset", "figure", "visit_simplelist"
      ...
      unless node.elements.empty?
        ...
        if only_text
          ...
          node.children.each_with_index do |child, i|
            ...
            unless i == 0 || first_line || (child.name == 'literallayout' || child.name == 'itemizedlist' || child.name == 'simplelist' || child.name == 'orderedlist')
              append_line '+'
              @continuation = true
            ...
            if (PARA_TAG_NAMES.include? child.name) || (child.name.eql? "text")
              ...
              if !FORMATTING_NAMES.include? child.name
                ...
                unless local_continuation || (child.name == 'literallayout' || child.name == 'itemizedlist' || child.name == 'simplelist' || child.name == 'orderedlist')
                  append_line '+'
                end
                @continuation = false
              end
